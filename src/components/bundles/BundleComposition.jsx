...
export default function BundleComposition({ pricing, lang = 'pl' }) {
  return (
    <div className="overflow-hidden rounded-[16px] border" style={{ borderColor: 'var(--color-border)' }}>
      <div className="grid gap-3 border-b px-5 py-4 text-[11px] font-medium uppercase tracking-[0.12em] text-fg-subtle sm:grid-cols-[1fr_90px_170px]" style={{ borderColor: 'var(--color-border)', background: 'var(--color-bg-muted)' }}>
      ...
      <div>
        {pricing.lineItems.map((item, i) => {
          ...
          return (
            <div key={item.productSlug} className={`grid gap-3 px-5 py-4 sm:grid-cols-[1fr_90px_170px] sm:items-center ${i ? 'border-t' : ''}`} style={{ borderColor: 'var(--color-border)' }}>
              <Link to={productRoutePath(item.product)} className="flex items-center gap-3 no-underline" style={{ color: 'var(--color-fg)' }}>
                <span
