...
import { ArrowRight, Building2, CheckCircle2, ClipboardList, ShieldCheck } from 'lucide-react';
...
const businessBundleSlugs = ['firma-podstawowa-4', 'firma-operacyjna-8', 'firma-gastro-12'];
const businessTypes = [
  'biura i coworki',
  'gabinety lekarskie, kosmetyczne i terapeutyczne',
  'salony fryzjerskie i beauty',
  'małe lokale gastronomiczne i usługowe',
  'warsztaty, punkty serwisowe i punkty obsługi klienta',
  'apartamenty, recepcje i zaplecza socjalne',
];

function BusinessBundle({ slug, index }) {
    ...
    <main className="pb-24 pt-[120px]" style={{ background: 'var(--color-bg)' }}>
      <section className="px-6 pb-20">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_430px] lg:items-center">
          <Motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
            <span className="t-eyebrow">Dla firm</span>
            <h1 className="t-display-1 mt-4">Stały zapas środków czystości dla małej firmy.</h1>
          ...
          <Motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.55, delay: 0.08 }} className="grid gap-4">
            ...
            <div className="rounded-[24px] border p-6" style={{ borderColor: 'var(--color-border)', background: 'var(--color-bg-muted)' }}>
              ...
              <p className="mt-3 text-[14px] leading-relaxed text-fg-muted">
                Dobrze dobrany karton porządkuje zakup chemii: recepcja, toaleta, kuchnia, zaplecze i powierzchnie wspólne mają własny zestaw.
              </p>
            </div>
          </Motion.div>
        </div>
      ...
      <section className="px-6 py-20" style={{ background: 'var(--color-bg-muted)' }}>
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 max-w-[700px]">
            ...
            <h2 className="t-h1 mt-4">Od podstawowego zapasu po zestaw operacyjny.</h2>
          </div>
          <div className="grid gap-5 lg:grid-cols-3">
            {businessBundleSlugs.map((slug, index) => <BusinessBundle key={slug} slug={slug} index={index} />)}
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-20">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.88fr_1.12fr]">
          <div>
            <span className="t-eyebrow">Zastosowania</span>
          ...
          <div>
            <span className="t-eyebrow">Produkty w paczkach firmowych</span>
            <h2 className="t-h2 mt-4">Bez prania i płukania, z naciskiem na strefy pracy.</h2>
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {businessProducts.map((product) => <ProductLine key={product.slug} product={product} />)}
            </div>
          </div>
        </div>
      </section>
