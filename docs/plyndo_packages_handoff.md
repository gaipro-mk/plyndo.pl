...
- navbar ma listę 12 produktów na desktopie i mobile, a podstrony produktu, pakietu, konfiguratora i informacji mają breadcrumbs,
...
- konfigurator wizualnie pokazuje wypełnienie kartonu; rabat pakietowy naliczany jest dopiero po skompletowaniu paczki 4/8/12,
- detale produktów oraz FAQ mają warstwę PL/EN; landing ma statyczne meta SEO/GEO plus dynamiczny title/description/canonical/og:image,
- sekcje `TrustSection` i `AiAssistantSection` są zamontowane na stronie głównej,
- przełączniki motywu, języka i skali fontu są dostępne w `TopNav`.

### 1.10 Remediacja audytu ultra (2026-06-30)

Po audycie `AUDYT-ULTRA-2026-06-30.md` wdrożono:

| Obszar | Stan |
| --- | --- |
| Routing SPA / QR | `public/_redirects` — 301 ze starych slugów + catch-all; długie slugi `/produkt/plyn-do-*` kanoniczne |
| SEO techniczne | `robots.txt`, `sitemap.xml` (generator), `llms.txt`, `_headers`, poprawiony `index.html` + JSON-LD |
| Prerender | `scripts/prerender.mjs` — 31 tras po buildzie |
| CTA sklepu | `src/lib/storeCta.js` — aktywny sklep tylko gdy `cta.status === 'enabled'` |
| Prawo / RODO | pełne treści PL w `InfoPage`; banner `CookieConsent` |
| Wydajność | code-splitting; hero — jedno aktywne wideo + poster |
| Dostępność | skip-link, focus-visible, reduced-motion, ARIA |
| Build | `npm run build` = vite + sitemap + prerender |

**Nadal wymaga decyzji biznesowej:** Shoper, finalne ceny (Omnibus), NIP/telefon, SDS/CLP, opinie, Starter 12.

**Kluczowe pliki:** `storeCta.js`, `prerender.mjs`, `generate-sitemap.mjs`, `CookieConsent.jsx`, `NotFoundPage.jsx`, `public/_redirects`.
