...
async function run() {
  console.log('🚀 Naprawa dat ważności kuponów (2027-12-31 / wyłączenie time_limit)...');
  ...
  for (const id of [4, 5, 6]) {
    console.log(`✏️ Poprawianie daty ważności dla kuponu ID ${id}...`);
    await page.goto(`https://sklep562393.shoparena.pl/admin/promoCodes/edit/id/${id}`, { waitUntil: 'networkidle' });

    // Uncheck time_limit if checked
    const timeLimitCb = page.locator('#time_limit').first();
    if (await timeLimitCb.isVisible() && await timeLimitCb.isChecked()) {
      await timeLimitCb.uncheck({ force: true });
    }

    // Set time_to to future date
    await page.evaluate(() => {
      const tTo = document.querySelector('input[name="time_to"]');
      if (tTo) tTo.value = '2027-12-31';
    });
    ...
    console.log(`   ✔ Poprawiono kupon ID ${id}!`);
  }

  console.log('--- WERYFIKACJA REST API DLA PLYNDO-PACK-4 ---');
