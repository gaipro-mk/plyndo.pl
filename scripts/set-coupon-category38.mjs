...
async function run() {
  console.log('🚀 Przypisywanie kategorii 38 ("Dom") do kuponów 4, 5, 6...');
  ...
  for (const id of [4, 5, 6]) {
    console.log(`✏️ Przypisywanie kategorii 38 dla kuponu ID ${id}...`);
    await page.goto(`https://sklep562393.shoparena.pl/admin/promoCodes/edit/id/${id}`, { waitUntil: 'networkidle' });

    // Select category 38 option / checkbox
    const ass1 = page.locator('#assortment_1').first();
    if (await ass1.isVisible()) {
      await ass1.check({ force: true });
      await page.waitForTimeout(500);
    }

    // Check category 38 checkbox
    const cat38 = page.locator('input[name="categories[]"][value="38"], #category_38, input[value="38"]').first();
    if (await cat38.isVisible()) {
      await cat38.check({ force: true });
    }
    ...
    console.log(`   ✔ Uaktualniono kategorie dla kuponu ID ${id}!`);
  }
