...
async function run() {
  console.log('🚀 Ustawianie opcji "Dla wszystkich" (#global_1) na kuponach 4, 5, 6...');
  ...
  for (const id of [4, 5, 6]) {
    console.log(`✏️ Ustawianie "Dla wszystkich" (#global_1) dla kuponu ID ${id}...`);
    await page.goto(`https://sklep562393.shoparena.pl/admin/promoCodes/edit/id/${id}`, { waitUntil: 'networkidle' });

    // Check global_1 (Dla wszystkich)
    const glob1 = page.locator('#global_1').first();
    if (await glob1.isVisible()) {
      await glob1.check({ force: true });
    }
    ...
    console.log(`   ✔ Uaktualniono kupon ID ${id}!`);
  }
