...
async function run() {
  console.log('🚀 Pobieranie motywu Shoper (Faza 2) z aktywną sesją przeglądarki...');
  ...
  console.log('Zalogowano pomyślnie. Adres URL:', page.url());

  // Navigate to skins list
  await page.goto('https://sklep562393.shoparena.pl/admin/skins/list', { waitUntil: 'networkidle' });
  await page.screenshot({ path: path.resolve(__dirname, '../docs/screenshots/skins-list-active.png'), fullPage: true });

  // Extract all skin IDs and names from page
  const skins = await page.evaluate(() => {
    const rows = Array.from(document.querySelectorAll('tr, .skin-item, .table-row, a'));
    return rows.map(r => ({
      text: r.innerText ? r.innerText.trim() : '',
      href: r.getAttribute ? r.getAttribute('href') : null
    })).filter(r => r.text);
  });

  console.log('Wyciągnięte dane motywów:', JSON.stringify(skins.slice(0, 20), null, 2));

  // Extract active skin ID from DOM or API
  const activeSkinId = '5df02446..._copy_2'; // known active theme ID from Shoper logs
  console.log('Aktywny ID motywu:', activeSkinId);
